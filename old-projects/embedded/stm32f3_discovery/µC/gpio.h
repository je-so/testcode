...
typedef enum gpio_port_bit_e {
   GPIO_PORTA_BIT = (uint8_t) (1u << 0),
   GPIO_PORTB_BIT = (uint8_t) (1u << 1),
   GPIO_PORTC_BIT = (uint8_t) (1u << 2),
   GPIO_PORTD_BIT = (uint8_t) (1u << 3),
   GPIO_PORTE_BIT = (uint8_t) (1u << 4),
   GPIO_PORTF_BIT = (uint8_t) (1u << 5)
} gpio_port_bit_e;
...
static inline void setpins_gpio(volatile struct gpio_port_t *port, uint16_t highpins/*1*/, uint16_t lowpins/*0*/);
