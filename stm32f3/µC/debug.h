...
#define hDBG    ((core_debug_t*) HW_BASEADDR_COREDEBUG)
...
static inline void enable_trace_dbg(void);      // enable trace units like DWT and ITM
static inline void disable_trace_dbg(void);
...
#define HW_BIT_SCS_DHCSR_S_REGRDY   (1u << 16)
#define HW_BIT_SCS_DHCSR_C_STEP     (1u << 2)   // (rw) 0: No effect. 1: Step the processor.
#define HW_BIT_SCS_DHCSR_C_HALT     (1u << 1)   // (rw) 0: No effect. 1: Halt the processor.
#define HW_BIT_SCS_DHCSR_C_DEBUGEN  (1u << 0)   // (rw) 1: Enable halting debug behavior ==> processor enters Debug state (used with external debugger)

// DEMCR: Debug Exception and Monitor Control Register
// not lsited bits are either reserved or only used in case DHCSR.C_DEBUGEN is set to 1 (used by external debugger)
#define HW_BIT_SCS_DEMCR_TRCENA     (1u << 24) // Global enable for all DWT and ITM features: 0: DWT and ITM blocks disabled. 1: DWT and ITM blocks enabled.
#define HW_BIT_SCS_DEMCR_MONREQ     (1u << 19) // Indicates Debug Monitor Exception was requested by Software
#define HW_BIT_SCS_DEMCR_MONSTEP    (1u << 18) // (only used if MONEN==1) 1: Sets MONPEND after every instruction. 0: Do not step the processor.
#define HW_BIT_SCS_DEMCR_MONPEND    (1u << 17) // Pending State of Debug Monitor Exception. 1: Pending
#define HW_BIT_SCS_DEMCR_MONEN      (1u << 16) // Indicates Debug Monitor Exception Enabled. 1: Enabled
...
static inline int isdebugger_dbg(void)
{
   return (hDBG->dhcsr / HW_BIT_SCS_DHCSR_C_DEBUGEN) & 1;
}
...
static inline int isenabled_trace_dbg(void)
{
   return (hDBG->demcr / HW_BIT_SCS_DEMCR_TRCENA) & 1;
}
...
{
   // TODO: atomic ?
   hDBG->demcr &= ~HW_BIT_SCS_DEMCR_MONPEND;
}
...
static inline void enable_trace_dbg(void)
{
   // TODO: atomic ?
   hDBG->demcr |= HW_BIT_SCS_DEMCR_TRCENA;
}
