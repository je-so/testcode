...
#define hMPU      ((core_mpu_t*) HW_REGISTER_BASEADDR_MPU)
...
static inline int  config_mpu(uint32_t nrregions/*<=nrregions_mpu*/, const struct mpu_region_t *regions/*[nrregions]*/, mpucfg_e cfg); // Configures MPU to use nrregions, other regions are disabled.
static inline uint32_t nextfreeregion_mpu(uint32_t firstnr);
static inline void update_mpu(uint32_t firstnr, uint32_t nrregions, const struct mpu_region_t *regions/*nrregions*/);
...
static inline int config_mpu(uint32_t nrregions, const mpu_region_t *regions, mpucfg_e cfg)
{
   ...
   disable_mpu();
   // config and enable first nrregions regions (assumes maxnr <= 16)
