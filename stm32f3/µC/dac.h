...
#define DAC1 ((dac_t*)HW_BASEADDR_DAC1)
...
#define HW_BIT_DAC_CR_DMAUDRIE2 (1u << 29)// DMAUDRIE2: DAC channel2 DMA underrun interrupt enable
#define HW_BIT_DAC_CR_DMAEN2  (1u << 28)  // DMAEN2: DAC channel2 DMA enable
#define HW_BIT_DAC_CR_TSEL2_POS     19u   // TSEL2[2:0]: DAC channel2 trigger selection (bits cannot be changed when the EN2 bit is set).
#define HW_BIT_DAC_CR_TSEL2_BITS    0x7
#define HW_BIT_DAC_CR_TSEL2_MASK    HW_REGISTER_MASK(DAC, CR, TSEL2)
#define HW_BIT_DAC_CR_TEN2    (1u << 18)  // TEN2: DAC channel2 trigger enable
#define HW_BIT_DAC_CR_BOFF2   (1u << 17)  // BOFF2: DAC channel2 output buffer disable
#define HW_BIT_DAC_CR_EN2     (1u << 16)  // EN2: DAC channel2 enable
#define HW_BIT_DAC_CR_DMAUDRIE1 (1u << 13)// DMAUDRIE1: DAC channel1 DMA Underrun Interrupt enable
#define HW_BIT_DAC_CR_DMAEN1  (1u << 12)  // DMAEN1: DAC channel1 DMA enable
#define HW_BIT_DAC_CR_TSEL1_POS     3u    // TSEL1[2:0]: DAC channel1 trigger selection (bits cannot be changed when the EN1 bit is set).
#define HW_BIT_DAC_CR_TSEL1_BITS    0x7
#define HW_BIT_DAC_CR_TSEL1_MASK    HW_REGISTER_MASK(DAC, CR, TSEL1)
#define HW_BIT_DAC_CR_TEN1    (1u << 2)   // TEN1: DAC channel1 trigger enable
#define HW_BIT_DAC_CR_BOFF1   (1u << 1)   // BOFF1: DAC channel1 output buffer disable
#define HW_BIT_DAC_CR_EN1     (1u << 0)   // EN1: DAC channel1 enable
...
static inline void config_dac(volatile struct dac_t *dac, dac_channel_e channel, daccfg_e config)
{
   ...
   if ((channel&dac_channel_1) != 0) {
      dac->cr &= ~HW_BIT_DAC_CR_EN1; // disable to allow change of configuration
      // for channel1: disable channel and trigger and enable output buffer
      cr &= ~( HW_BIT_DAC_CR_DMAUDRIE1    // disable channel1 DMA Underrun Interrupt
               |HW_BIT_DAC_CR_DMAEN1      // disable DAC channel1 DMA
               |HW_BIT_DAC_CR_TSEL1_MASK  // clear DAC channel1 trigger selection
               |HW_BIT_DAC_CR_TEN1        // disable DAC channel1 trigger
               |HW_BIT_DAC_CR_BOFF1       // enable DAC channel1 output buffer
               |HW_BIT_DAC_CR_EN1         // disable DAC channel1 enable
               );
      cr |= (config & daccfg_TRIGGER_MASK) << HW_BIT_DAC_CR_TSEL1_POS;
      static_assert(daccfg_DMA < HW_BIT_DAC_CR_DMAEN1);
      cr |= (config & daccfg_DMA) * (HW_BIT_DAC_CR_DMAEN1/daccfg_DMA);
      static_assert(daccfg_ENABLE_TRIGGER > HW_BIT_DAC_CR_TEN1);
      cr |= (config & daccfg_ENABLE_TRIGGER) / (daccfg_ENABLE_TRIGGER/HW_BIT_DAC_CR_TEN1);
      static_assert(daccfg_ENABLE_CHANNEL > HW_BIT_DAC_CR_EN1);
      cr |= (config & daccfg_ENABLE_CHANNEL) / (daccfg_ENABLE_CHANNEL/HW_BIT_DAC_CR_EN1);
   }
   if ((channel&dac_channel_2) != 0) {
      dac->cr &= ~HW_BIT_DAC_CR_EN2; // disable to allow change of configuration
      cr &= ~( HW_BIT_DAC_CR_DMAUDRIE2    // disable channel2 DMA Underrun Interrupt
               |HW_BIT_DAC_CR_DMAEN2      // disable DAC channel2 DMA
               |HW_BIT_DAC_CR_TSEL2_MASK  // clear DAC channel2 trigger selection
               |HW_BIT_DAC_CR_TEN2        // disable DAC channel2 trigger
               |HW_BIT_DAC_CR_BOFF2       // enable DAC channel2 output buffer
               |HW_BIT_DAC_CR_EN2         // disable DAC channel2 enable
               );
      cr |= (config & daccfg_TRIGGER_MASK) << HW_BIT_DAC_CR_TSEL2_POS;
      static_assert(daccfg_DMA < HW_BIT_DAC_CR_DMAEN2);
      cr |= (config & daccfg_DMA) * (HW_BIT_DAC_CR_DMAEN2/daccfg_DMA);
      static_assert(daccfg_ENABLE_TRIGGER < HW_BIT_DAC_CR_TEN2);
      cr |= (config & daccfg_ENABLE_TRIGGER) * (HW_BIT_DAC_CR_TEN2/daccfg_ENABLE_TRIGGER);
      static_assert(daccfg_ENABLE_CHANNEL < HW_BIT_DAC_CR_EN2);
      cr |= (config & daccfg_ENABLE_CHANNEL) * (HW_BIT_DAC_CR_EN2/daccfg_ENABLE_CHANNEL);
   }
   dac->cr = cr;
}
...
static inline void enable_dac(dac_t *dac, dac_channel_e channel)
{
   uint32_t cr = dac->cr;
   if ((channel&dac_channel_1) != 0) {
      cr |= HW_BIT_DAC_CR_EN1;
   }
   if ((channel&dac_channel_2) != 0) {
      cr |= HW_BIT_DAC_CR_EN2;
   }
   dac->cr = cr;
}
...
{
   uint32_t cr = dac->cr;
   uint32_t mask = (channel&dac_channel_1) != 0 ? HW_BIT_DAC_CR_EN1 : 0;
   if ((channel&dac_channel_2) != 0) {
      mask |= HW_BIT_DAC_CR_EN2;
   }
   return (cr & mask) == mask;
}
