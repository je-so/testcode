...
static volatile uint8_t _Alignas(1024) aligned_data[1024];
...
int main(void)
{
   ...
   enable_trace_dbg();     // enable dwt debug component which supports counter and watchpoints
   ...
   for (;;) {
      ...
      // TEST addwatchpoint_dwtdbg: address range with masking address bits
      static_assert(sizeof(aligned_data) == 1 << 10);
      for (uint32_t ignore_lsb = 0; ignore_lsb <= 10; ++ignore_lsb) {
         err = addwatchpoint_dwtdbg(&wp1, dwtdbg_watchpoint_DATAADDR_RO, (uintptr_t)&aligned_data[0], ignore_lsb);
         assert( 0 == err);
         for (uint32_t i = 0; i < 100; ++i) {
            assert( i == debugcounter);
            assert( 0 == ismatch_dwtdbg(wp1));
            (void) aligned_data[i%(1<<ignore_lsb)];   // read ACCESS in address range ==> debug event
            for (volatile int w = 0; w < 10; ++w) ;
            assert( i+1 == debugcounter);
            assert( 1 == ismatch_dwtdbg(wp1));        // clears value
            assert( 0 == ismatch_dwtdbg(wp1));
         }
         (void) aligned_data[(1<<ignore_lsb)];  // not in address range ==> ignored
         for (volatile int w = 0; w < 10; ++w) ;
         assert( 100 == debugcounter);
         // reset
         clearwatchpoint_dwtdbg(wp1);
         debugcounter = 0;
      }
