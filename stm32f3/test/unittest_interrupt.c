...
int unittest_interrupt(void)
{
   ...
   // TEST isenabled_interruptEINVAL
   ...
   // Test getpriority_interruptEINVAL
   ...
   for (uint32_t i = 16; i <= HW_KONFIG_NVIC_INTERRUPT_MAXNR; ++i) {
      const uint8_t M = interrupt_priority_MIN;
      assert( 0 == getpriority_interrupt(i)); // default after reset
      assert( 0 == setpriority_interrupt(i, M));
      assert( M == getpriority_interrupt(i)); // LOW priority set
   }

   // TEST getpriority_interrupt
   for (uint32_t i = 16; i <= HW_KONFIG_NVIC_INTERRUPT_MAXNR; ++i) {
      const uint8_t M = interrupt_priority_MIN;
      assert( M == getpriority_interrupt(i)); // default after reset
      assert( 0 == setpriority_interrupt(i, interrupt_priority_MAX));
      assert( 0 == getpriority_interrupt(i)); // HIGH priority set
   }
   ...
   setpriority_interrupt(interrupt_DMA2_CHANNEL3, interrupt_priority_MIN);
