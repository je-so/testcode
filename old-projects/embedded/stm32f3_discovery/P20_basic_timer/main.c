...
int main(void)
{
   ...
   for (unsigned clock = 0; 1/*repeat forever*/; ++clock) {
      ...
      for (unsigned t = 0; t < 2; ++t) {
         ...
         switch_used_led();
         config_systick(65000, systickcfg_CORECLK);
         ...
         for (int i = 0; i < 2; ++i) {
            switch_used_led();
            if (i == 0) {
               config_systick(2*65000, systickcfg_CORECLK);
               assert(config_basictimer(timer, 2, 65000, basictimercfg_INTERRUPT) == 0);
            } else {
               config_systick(50000, systickcfg_CORECLK);
         ...
         switch_used_led();
         config_systick(60000, systickcfg_CORECLK);
         ...
         config_systick(s_main.count, systickcfg_CORECLK); // Warte Restperiode
         ...
         config_systick(2*65536-60000, systickcfg_CORECLK); // Warte Restperiode
         continue_basictimer(timer);                        // continue Timer
         start_systick();                                   // schalte Systick nach Timer an
         while (!isexpired_systick()) ;                     // Warte bis Systick abgelaufen (Restzeit)
         ...
         switch_used_led();
         config_systick(10000, systickcfg_CORECLK);
         ...
         switch_used_led();
         config_systick(0xffffff, systickcfg_CORECLK);
