 ...
 * Byte accessible. Only upper HW_KONFIG_NVIC_INTERRUPT_PRIORITY_NROFBITS bits of every byte are changeable
...
static inline int setpriority_interrupt_nvic(interrupt_e intnr, interrupt_priority_e priority)
{
   ...
   ipr[intnr] = (uint8_t) (priority << (8-HW_KONFIG_NVIC_INTERRUPT_PRIORITY_NROFBITS));
...
static inline uint8_t getpriority_interrupt_nvic(interrupt_e intnr)
{
   ...
   volatile uint8_t *ipr = (volatile uint8_t*)& HW_REGISTER(NVIC, IPR);
   return ipr[intnr] >> (8-HW_KONFIG_NVIC_INTERRUPT_PRIORITY_NROFBITS);
}
