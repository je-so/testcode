...
#define WRITE_BIT(NR) \
      ...
      setpins_gpio(LCD_PORT, (uint16_t) bit, (uint16_t) (bit^LCD_MOSI)); \
...
void init_lcd(void)
{
   ...
   config_systick(HZ/(1000000/10)/*10µs*/, systickcfg_CORECLK);
   ...
   config_systick(HZ/(1000/20)/*20ms*/, systickcfg_CORECLK);
	start_systick();
   while (!isexpired_systick()) ;
	stop_systick();
   ...
   config_systick(HZ/(1000/60)/*60ms*/, systickcfg_CORECLK);
	start_systick();
   while (!isexpired_systick()) ;
	stop_systick();
