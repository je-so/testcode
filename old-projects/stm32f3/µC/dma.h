...
#define DMA1   ((dma_t*)HW_REGISTER_BASEADDR_DMA1)
#define DMA2   ((dma_t*)HW_REGISTER_BASEADDR_DMA2)
...
static inline void assert_dmabits_dma(void)
{
   ...
   static_assert(HW_REGISTER_BIT_RCC_AHBENR_DMA1EN == DMA1_BIT);
   static_assert(HW_REGISTER_BIT_RCC_AHBENR_DMA2EN == DMA2_BIT);

   static_assert(dma_state_ERROR    == HW_REGISTER_BIT_DMA_ISR_TEIF1);
   static_assert(dma_state_HALF     == HW_REGISTER_BIT_DMA_ISR_HTIF1);
   static_assert(dma_state_COMPLETE == HW_REGISTER_BIT_DMA_ISR_TCIF1);
}
...
{
   static_assert(dmacfg_PRIORITY_MAX == 3 << HW_REGISTER_BIT_DMA_CCR_PL_POS);
   static_assert(dmacfg_PRIORITY_HIGH == 2 << HW_REGISTER_BIT_DMA_CCR_PL_POS);
   static_assert(dmacfg_PRIORITY_LOW == 1 << HW_REGISTER_BIT_DMA_CCR_PL_POS);
   static_assert(dmacfg_PRIORITY_MIN == 0 << HW_REGISTER_BIT_DMA_CCR_PL_POS);
   static_assert(dmacfg_MEM_8BITDATA  == 0 << HW_REGISTER_BIT_DMA_CCR_MSIZE_POS);
   static_assert(dmacfg_MEM_16BITDATA == 1 << HW_REGISTER_BIT_DMA_CCR_MSIZE_POS);
   static_assert(dmacfg_MEM_32BITDATA == 2 << HW_REGISTER_BIT_DMA_CCR_MSIZE_POS);
   static_assert(dmacfg_HW_8BITDATA  == 0 << HW_REGISTER_BIT_DMA_CCR_PSIZE_POS);
   static_assert(dmacfg_HW_16BITDATA == 1 << HW_REGISTER_BIT_DMA_CCR_PSIZE_POS);
   static_assert(dmacfg_HW_32BITDATA == 2 << HW_REGISTER_BIT_DMA_CCR_PSIZE_POS);
   static_assert(dmacfg_NOTRIGGER          == HW_REGISTER_BIT_DMA_CCR_MEM2MEM);
   static_assert(dmacfg_MEM_INCRADDR == HW_REGISTER_BIT_DMA_CCR_MINC);
   static_assert(dmacfg_HW_INCRADDR  == HW_REGISTER_BIT_DMA_CCR_PINC);
   static_assert(dmacfg_LOOP      == HW_REGISTER_BIT_DMA_CCR_CIRC);
   static_assert(dmacfg_MEM_READ  == HW_REGISTER_BIT_DMA_CCR_DIR);
   static_assert(dmacfg_MEM_WRITE == 0);
   static_assert(dmacfg_ENABLE == HW_REGISTER_BIT_DMA_CCR_EN);
   static_assert(dmacfg_INTERRUPT_ERROR    == HW_REGISTER_BIT_DMA_CCR_TEIE);
   static_assert(dmacfg_INTERRUPT_HALF     == HW_REGISTER_BIT_DMA_CCR_HTIE);
   static_assert(dmacfg_INTERRUPT_COMPLETE == HW_REGISTER_BIT_DMA_CCR_TCIE);
}
...
static inline int config_dma(dma_t *dma, dma_channel_e channel, volatile void *hwunit/*_HW_*/, volatile void *memory/*_MEM_*/, uint16_t datacount, dmacfg_e config)
{
   ...
   dma->channel[channel].ccr &= ~HW_REGISTER_BIT_DMA_CCR_EN;

   // clear all interrupt flags for this channel
   dma->ifcr = HW_REGISTER_BIT_DMA_ISR_CHANNEL_MASK << (channel * HW_REGISTER_BIT_DMA_ISR_CHANNEL_BITS);
...
static inline int isenabled_dma(const dma_t *dma, dma_channel_e channel)
{
   ...
   return (dma->channel[channel].ccr & HW_REGISTER_BIT_DMA_CCR_EN) / HW_REGISTER_BIT_DMA_CCR_EN;
}
...
{
   return (dma->isr >> (channel * HW_REGISTER_BIT_DMA_ISR_CHANNEL_BITS)) & (HW_REGISTER_BIT_DMA_ISR_CHANNEL_MASK-HW_REGISTER_BIT_DMA_ISR_GIF1);
}
...
{
   dma->ifcr = (uint32_t)state << (channel * HW_REGISTER_BIT_DMA_ISR_CHANNEL_BITS);
}
