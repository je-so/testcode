/* title: Analog-to-Digital-Converter
  ...
  /Pinout/ Kanäle:
   ...
   HW_BIT_ADC_CFGR_AUTDLY die Konvertierung des nächsten Kanals einer Seqeuenz solange
...
#define ADC1      ((volatile struct adc_t*)(HW_BASEADDR_ADC1))
#define ADC2      ((volatile struct adc_t*)(HW_BASEADDR_ADC1 + 0x100))
#define ADC3      ((volatile struct adc_t*)(HW_BASEADDR_ADC3))
#define ADC4      ((volatile struct adc_t*)(HW_BASEADDR_ADC3 + 0x100))
#define ADC1and2  ((volatile struct adc_dual_t*)(HW_BASEADDR_ADC1 + 0x300))
#define ADC3and4  ((volatile struct adc_dual_t*)(HW_BASEADDR_ADC3 + 0x300))
...
#define HW_BIT_ADC_ISR_JQOVF  (1u << 10)  // 1: Injected context queue overflow has occurred
#define HW_BIT_ADC_ISR_AWD3   (1u << 9)   // 1: Analog watchdog 3 event occurred
#define HW_BIT_ADC_ISR_AWD2   (1u << 8)   // 1: Analog watchdog 2 event occurred
#define HW_BIT_ADC_ISR_AWD1   (1u << 7)   // 1: Analog watchdog 1 event occurred
#define HW_BIT_ADC_ISR_JEOS   (1u << 6)   // 1: Injected conversions complete (end of injected sequence)
#define HW_BIT_ADC_ISR_JEOC   (1u << 5)   // 1: Injected channel conversion complete (end of injected conversion)
#define HW_BIT_ADC_ISR_OVR    (1u << 4)   // 1: ADC Overrun has occurred
#define HW_BIT_ADC_ISR_EOS    (1u << 3)   // 1: Regular Conversions sequence complete (end of regular sequence)
#define HW_BIT_ADC_ISR_EOC    (1u << 2)   // 1: Regular channel conversion complete (end of regular conversion)
#define HW_BIT_ADC_ISR_EOSMP  (1u << 1)   // 1: End of sampling phase reached (RC-circuit is precharged with Vin)
#define HW_BIT_ADC_ISR_ADRDY  (1u << 0)   // 1: ADC is ready to start conversion (set by hardware after ADEN=1)

#define HW_BIT_ADC_CR_ADCAL   (1u << 31)  // Write 1 to calibrate the ADC. Read at 1 means that a calibration in progress.
#define HW_BIT_ADC_CR_ADCALDIF (1u << 30) // 0: Single-ended inputs Mode Calibration. 1: Differential inputs Mode Calibration.
#define HW_BIT_ADC_CR_ADVREGEN_POS  28u   // 00: Intermediate state; 01: ADC Voltage regulator enabled;
#define HW_BIT_ADC_CR_ADVREGEN_BITS 3u    // 10: ADC Voltage regulator disabled (Reset state); 11: reserved
#define HW_BIT_ADC_CR_ADVREGEN_MASK HW_REGISTER_MASK(ADC, CR, ADVREGEN)
#define HW_BIT_ADC_CR_JADSTP  (1u << 5)   // Write 1: stop injected conversions. Read 1: JADSTP command is in progress.
#define HW_BIT_ADC_CR_ADSTP   (1u << 4)   // Write 1: stop regular conversions. Read 1: ADSTP command is in progress.
#define HW_BIT_ADC_CR_JADSTART (1u << 3)  // Write 1: start injected conversions. Read 1: ADC operating and eventually converting injected channels
#define HW_BIT_ADC_CR_ADSTART (1u << 2)   // Write 1: start regular conversions. Read 1: ADC operating and eventually converting regular channels
#define HW_BIT_ADC_CR_ADDIS   (1u << 1)   // Write 1: disable the ADC. Read 1: an ADDIS command is in progress
#define HW_BIT_ADC_CR_ADEN    (1u << 0)   // 1: ADC enabled or enable ADC.

#define HW_BIT_ADC_CFGR_AWD1CH_POS  26u   // [4:0]: Analog watchdogHW_BIT_ADC_SQR1_SQ4_POS 1 channel selection
#define HW_BIT_ADC_CFGR_AWD1CH_BITS 0x1f
#define HW_BIT_ADC_CFGR_AWD1CH_MASK HW_REGISTER_MASK(ADC, CFGR, AWD1CH)
#define HW_BIT_ADC_CFGR_JAUTO (1u << 25)  // 1: enabled Automatic injected group conversion (after regular conversion)
#define HW_BIT_ADC_CFGR_JAWD1EN (1u << 24) // 1: enabled Analog watchdog 1 on injected channels
#define HW_BIT_ADC_CFGR_AWD1EN (1u << 23) // 1: enabled Analog watchdog 1 on regular channels
#define HW_BIT_ADC_CFGR_AWD1SGL (1u << 22) // 1: single channel Analog 1 watchdog (AWD1CH) 0: Analog watchdog 1 enabled on all channels
#define HW_BIT_ADC_CFGR_JQM   (1u << 21)  // JSQR queue mode 0: maintains last written state of JSQR 1: could get empty and conversion stops
#define HW_BIT_ADC_CFGR_JDISCEN (1u << 20) // 1: Discontinuous mode on injected channels enabled
#define HW_BIT_ADC_CFGR_DISCNUM_POS  17u  // [2:0]: Discontinuous mode channel count
#define HW_BIT_ADC_CFGR_DISCNUM_BITS 0x7  // Es werden nur DISCNUM Werte einer Sequenz konvertiert. Aus einer Seqeuenz von Werten werden so x Untergruppen mit DISCNUM Werten oder am Ende der Sequenz weniger.
#define HW_BIT_ADC_CFGR_DISCNUM_MASK HW_REGISTER_MASK(ADC, CFGR, DISCNUM)
#define HW_BIT_ADC_CFGR_DISCEN  (1u << 16) // 1: Discontinuous mode for regular channels enabled
#define HW_BIT_ADC_CFGR_AUTDLY  (1u << 14) // 1: Auto-delayed conversion mode on
#define HW_BIT_ADC_CFGR_CONT  (1u << 13)  // 1: Continuous conversion mode (sequence repeated) 0: Single conversion mode (stop at end of sequence)
#define HW_BIT_ADC_CFGR_OVRMOD (1u << 12) // 1: DR register is overwritten with the last conversion result when an overrun is detected 0: preserved with the old data
#define HW_BIT_ADC_CFGR_EXTEN_POS  10u    // [1:0]: External trigger enable and polarity selection for regular channels
#define HW_BIT_ADC_CFGR_EXTEN_BITS 0x3    // 00: Software Trigger 01: rising edge HW Trigger 10: falling edge HW Trigger 11: both edges HW Trigger
#define HW_BIT_ADC_CFGR_EXTEN_MASK HW_REGISTER_MASK(ADC, CFGR, EXTEN)
#define HW_BIT_ADC_CFGR_EXTSEL_POS  6u    // [3:0]: External trigger selection for regular group
#define HW_BIT_ADC_CFGR_EXTSEL_BITS 0xf   // Selektiert Event 0 bis Event 15, unterschiedliche für ADC12 und ADC34, meist interne Events von Timern, aber auch ein EXTI PIN
#define HW_BIT_ADC_CFGR_EXTSEL_MASK HW_REGISTER_MASK(ADC, CFGR, EXTSEL)
#define HW_BIT_ADC_CFGR_ALIGN (1u << 5)   // Data alignment of DR register 1: Left alignment 0: Right alignment
#define HW_BIT_ADC_CFGR_RES_POS  3u       // [1:0]: Data resolution
#define HW_BIT_ADC_CFGR_RES_BITS 0x3      // 00: 12-Bit 01: 10-Bit 10: 8-Bit 11: 6-Bit
#define HW_BIT_ADC_CFGR_RES_MASK HW_REGISTER_MASK(ADC, CFGR, RES)
#define HW_BIT_ADC_CFGR_RES_12BIT (0x0 << HW_BIT_ADC_CFGR_RES_POS)
#define HW_BIT_ADC_CFGR_RES_10BIT (0x1 << HW_BIT_ADC_CFGR_RES_POS)
#define HW_BIT_ADC_CFGR_RES_8BIT  (0x2 << HW_BIT_ADC_CFGR_RES_POS)
#define HW_BIT_ADC_CFGR_RES_6BIT  (0x3 << HW_BIT_ADC_CFGR_RES_POS)
#define HW_BIT_ADC_CFGR_DMACFG (1u << 1)  // 1: DMA Circular Mode selected 0: DMA One Shot Mode selected
#define HW_BIT_ADC_CFGR_DMAEN  (1u << 0)  // 1: DMA enabled

#define HW_BIT_ADC_SMPR_BITS        0x7
#define HW_BIT_ADC_SMPR_1_5_CYCLES  0     // 1.5 ADC clock cycles
#define HW_BIT_ADC_SMPR_2_5_CYCLES  1     // 2.5 ADC clock cycles
#define HW_BIT_ADC_SMPR_4_5_CYCLES  2     // 4.5 ADC clock cycles
#define HW_BIT_ADC_SMPR_7_5_CYCLES  3     // 7.5 ADC clock cycles
#define HW_BIT_ADC_SMPR_19_5_CYCLES 4     // 19.5 ADC clock cycles
#define HW_BIT_ADC_SMPR_61_5_CYCLES 5     // 61.5 ADC clock cycles
#define HW_BIT_ADC_SMPR_181_5_CYCLES 6    // 181.5 ADC clock cycles
#define HW_BIT_ADC_SMPR_601_5_CYCLES 7    // 601.5 ADC clock cycles

#define HW_BIT_ADC_SQRX_CHANNEL_LENGTH 6u    // Ein Feld in den Registern SQR1..SQR4 beginnt immer ab einer durch 6 teilbaren Bitposition
#define HW_BIT_ADC_SQRX_CHANNEL_BITS   0x1F  // Bits [4:0] enthalten die Kanalnummer 1 .. 18 (Wert 0 ist unverbunden)
                                                      // Bit [5] ist reserviert
#define HW_BIT_ADC_SQRX_NRCHANNEL      5u    // Die Register SQR2..SQR4 enthalten jeweils 5 Kanalfelder,
                                                      // Das Register SQR1 enthält die Länge der Sequenz und daher nur 4 Kanäle

#define HW_BIT_ADC_SQR1_SQ4_POS     24u   // [4:0]: 4nd conversion in regular sequence
#define HW_BIT_ADC_SQR1_SQ4_BITS    0x1F  // Enthält Kanalnummer 1 .. 18 (Wert 0 nicht definiert)
#define HW_BIT_ADC_SQR1_SQ4_MASK    HW_REGISTER_MASK(ADC, SQR1, SQ4)
#define HW_BIT_ADC_SQR1_SQ3_POS     18u   // [4:0]: 3nd conversion in regular sequence
#define HW_BIT_ADC_SQR1_SQ3_BITS    0x1F  // Enthält Kanalnummer 1 .. 18 (Wert 0 nicht definiert)
#define HW_BIT_ADC_SQR1_SQ3_MASK    HW_REGISTER_MASK(ADC, SQR1, SQ3)
#define HW_BIT_ADC_SQR1_SQ2_POS     12u   // [4:0]: 2nd conversion in regular sequence
#define HW_BIT_ADC_SQR1_SQ2_BITS    0x1F  // Enthält Kanalnummer 1 .. 18 (Wert 0 nicht definiert)
#define HW_BIT_ADC_SQR1_SQ2_MASK    HW_REGISTER_MASK(ADC, SQR1, SQ2)
#define HW_BIT_ADC_SQR1_SQ1_POS     6u    // [4:0]: 1st conversion in regular sequence
#define HW_BIT_ADC_SQR1_SQ1_BITS    0x1F  // Enthält Kanalnummer 1 .. 18 (Wert 0 nicht definiert)
#define HW_BIT_ADC_SQR1_SQ1_MASK    HW_REGISTER_MASK(ADC, SQR1, SQ1)
#define HW_BIT_ADC_SQR1_L_POS       0u    // [3:0]: Regular channel sequence length
#define HW_BIT_ADC_SQR1_L_BITS      0xF   // 0000: 1 conversion ... 1111: 16 conversions
#define HW_BIT_ADC_SQR1_L_MASK      HW_REGISTER_MASK(ADC, SQR1, L)

#define HW_BIT_ADC_JSQR_JSQ4_POS    26u   // [4:0]: 4th conversion in the injected sequence
#define HW_BIT_ADC_JSQR_JSQ4_BITS   0x1f  // Enthält Kanalnummer 1 .. 18 (Wert 0 nicht belegt)
#define HW_BIT_ADC_JSQR_JSQ4_MASK   HW_REGISTER_MASK(ADC, JSQR, JSQ4)
#define HW_BIT_ADC_JSQR_JSQ3_POS    20u   // [4:0]: 3th conversion in the injected sequence
#define HW_BIT_ADC_JSQR_JSQ3_BITS   0x1f  // Enthält Kanalnummer 1 .. 18 (Wert 0 nicht belegt)
#define HW_BIT_ADC_JSQR_JSQ3_MASK   HW_REGISTER_MASK(ADC, JSQR, JSQ3)
#define HW_BIT_ADC_JSQR_JSQ2_POS    14u   // [4:0]: 2nd conversion in the injected sequence
#define HW_BIT_ADC_JSQR_JSQ2_BITS   0x1f  // Enthält Kanalnummer 1 .. 18 (Wert 0 nicht belegt)
#define HW_BIT_ADC_JSQR_JSQ2_MASK   HW_REGISTER_MASK(ADC, JSQR, JSQ2)
#define HW_BIT_ADC_JSQR_JSQ1_POS    8u    // [4:0]: 1st conversion in the injected sequence
#define HW_BIT_ADC_JSQR_JSQ1_BITS   0x1f  // Enthält Kanalnummer 1 .. 18 (Wert 0 nicht belegt)
#define HW_BIT_ADC_JSQR_JSQ1_MASK   HW_REGISTER_MASK(ADC, JSQR, JSQ1)
#define HW_BIT_ADC_JSQR_JEXTEN_POS  6u    // [1:0]: External Trigger Enable and Polarity Selection for injected channels
#define HW_BIT_ADC_JSQR_JEXTEN_BITS 0x3   // 00: Software Trigger 01: rising edge HW Trigger 10: falling edge HW Trigger 11: both edges HW Trigger
#define HW_BIT_ADC_JSQR_JEXTEN_MASK HW_REGISTER_MASK(ADC, JSQR, JEXTEN)
#define HW_BIT_ADC_JSQR_JEXTSEL_POS 2u    // [3:0]: External Trigger Selection for injected group
#define HW_BIT_ADC_JSQR_JEXTSEL_BITS 0xf  // Die Eventnummer von 0 bis 15, falls HW-Trigger in JEXTEN selektiert
#define HW_BIT_ADC_JSQR_JEXTSEL_MASK HW_REGISTER_MASK(ADC, JSQR, JEXTSEL)
#define HW_BIT_ADC_JSQR_JL_POS      0u    // [1:0]: Injected channel sequence length
#define HW_BIT_ADC_JSQR_JL_BITS     0x3   // 00: 1 Konvertierung 01: 2 Konv. 10: 3 Konv. 11: 4 Konvertierungen
#define HW_BIT_ADC_JSQR_JL_MASK     HW_REGISTER_MASK(ADC, JSQR, JL)

#define HW_BIT_ADC_OFR_EN     (1u << 31)  // 1: Offset is enabled
#define HW_BIT_ADC_OFR_CH_POS       26u   // [4:0]: Channel selection for the data offset
#define HW_BIT_ADC_OFR_CH_BITS      0x1f  // Kanalnummer 1 bis 18, Wert 0 ist keinem Kanal zugeordnet
#define HW_BIT_ADC_OFR_CH_MASK      HW_REGISTER_MASK(ADC, OFR, CH)
#define HW_BIT_ADC_OFR_OFFSET_POS   0u    // [11:0]: Data offset for the channel programmed into bits CH[4:0]
#define HW_BIT_ADC_OFR_OFFSET_BITS  0xfff // Dieser Wert wird vom konvertierten Wert abgezogen
#define HW_BIT_ADC_OFR_OFFSET_MASK  HW_REGISTER_MASK(ADC, OFR, OFFSET)

#define HW_BIT_ADC_CCR_VBATEN (1u << 24)  // 1: V BAT enabled
#define HW_BIT_ADC_CCR_TSEN   (1u << 23)  // 1: Temperature sensor enabled
#define HW_BIT_ADC_CCR_VREFEN (1u << 22)  // 1: V REFINT enabled
#define HW_BIT_ADC_CCR_CKMODE_POS   16u   // [1:0]: ADC clock mode; 00: PLL Asynchronous clock mode,
#define HW_BIT_ADC_CCR_CKMODE_BITS  0x3   // 01: HCLK/1 (Synchronous clock mode), 10: HCLK/2 (Synchronous clock mode), 11: HCLK/4 (Synchronous clock mode)
#define HW_BIT_ADC_CCR_CKMODE_MASK  HW_REGISTER_MASK(ADC, CCR, CKMODE)
...
static inline void enable_clock_adc(adc_dual_t *adc)
{
   ...
   ccr &= ~HW_BIT_ADC_CCR_CKMODE_MASK;
   ccr |= (1u << HW_BIT_ADC_CCR_CKMODE_POS); // 01: HCLK/1 (Synchronous clock mode)
...
{
   adc->ccr &= ~HW_BIT_ADC_CCR_CKMODE_MASK;
...
{
   uint32_t cr = adc->cr;
   if (((cr >> HW_BIT_ADC_CR_ADVREGEN_POS) & HW_BIT_ADC_CR_ADVREGEN_BITS) != 1) {
      // 00: Intermediate state
      cr &= ~HW_BIT_ADC_CR_ADVREGEN_MASK;
      adc->cr = cr;
      // 01: ADC Voltage regulator enabled;
      cr |= (1 << HW_BIT_ADC_CR_ADVREGEN_POS);
      adc->cr = cr;
...
{
   uint32_t cr = adc->cr;
   if (((cr >> HW_BIT_ADC_CR_ADVREGEN_POS) & HW_BIT_ADC_CR_ADVREGEN_BITS) != 2) {
      // 00: Intermediate state
      cr &= ~HW_BIT_ADC_CR_ADVREGEN_MASK;
      adc->cr = cr;
      // 10: ADC Voltage regulator disabled (Reset state);
      cr |= (2 << HW_BIT_ADC_CR_ADVREGEN_POS);
      adc->cr = cr;
   }
}
 ...
 *
 * Unchecked Precondition:
 * HW_BIT_ADC_CR_ADVREGEN == (1 << HW_BIT_ADC_CR_ADVREGEN_POS)
 *
 * Returns:
 * 0     - OK
...
{
   if ((adc->cr & HW_BIT_ADC_CR_ADEN) != 0) {
      return EBUSY;
   }

   adc->cr |= HW_BIT_ADC_CR_ADCALDIF; // 1: Differential inputs Mode Calibration
   adc->cr |= HW_BIT_ADC_CR_ADCAL;
   // wait for differential calibration to complete
   while ((adc->cr & HW_BIT_ADC_CR_ADCAL) != 0) ;

   adc->cr &= ~HW_BIT_ADC_CR_ADCALDIF; // 0: Calibration is for Single-ended inputs Mode
   adc->cr |= HW_BIT_ADC_CR_ADCAL;
...
{
   uint32_t cr = adc->cr;
   if ((cr & (HW_BIT_ADC_CR_JADSTART|HW_BIT_ADC_CR_ADSTART)) != 0) {
      if ((cr & HW_BIT_ADC_CR_ADSTART) != 0) {
         cr |= HW_BIT_ADC_CR_ADSTP;
      }
      if ((cr & HW_BIT_ADC_CR_JADSTART) != 0) {
         cr |= HW_BIT_ADC_CR_JADSTP;
      }
      adc->cr = cr;
      while ((adc->cr & (HW_BIT_ADC_CR_ADSTP|HW_BIT_ADC_CR_JADSTP)) != 0) ;
   }
}
...
{
   uint32_t cr = adc->cr;
   if ((cr & HW_BIT_ADC_CR_ADEN) == 0) {
      cr |= HW_BIT_ADC_CR_ADEN;
      adc->cr = cr;
      // wait until adc is ready to start conversion
      while ((adc->isr & HW_BIT_ADC_ISR_ADRDY) == 0) ;
   }
}
...
static inline void disable_adc(adc_t *adc)
{
   ...
   uint32_t cr = adc->cr;
   if ((cr & HW_BIT_ADC_CR_ADEN) != 0) {
      if ((cr & HW_BIT_ADC_CR_ADDIS) == 0) {
         // stop is not in progress ==> start stop process
         cr |= HW_BIT_ADC_CR_ADDIS;
         adc->cr = cr;
      }
      do {
         cr = adc->cr;
      } while ((cr & HW_BIT_ADC_CR_ADEN) != 0) ;
   }
}

static inline int isenabled_adc(volatile struct adc_t *adc)
{
   return (adc->cr & HW_BIT_ADC_CR_ADEN) / HW_BIT_ADC_CR_ADEN;
}
 ...
 *
 * Unchecked Precondition:
 * (HW_BIT_ADC_CR_ADSTART == 0) && (HW_BIT_ADC_CR_JADSTART == 0) */
static inline void setsampletime_adc(adc_t *adc, adc_chan_e chan, adc_config_e time/*time&adc_config_MASK_SAMPLETIME*/)
{
   static_assert( adc_config_SAMPLETIME_1_5 == HW_BIT_ADC_SMPR_1_5_CYCLES
                  && adc_config_SAMPLETIME_2_5 == HW_BIT_ADC_SMPR_2_5_CYCLES
                  && adc_config_SAMPLETIME_4_5 == HW_BIT_ADC_SMPR_4_5_CYCLES
                  && adc_config_SAMPLETIME_7_5 == HW_BIT_ADC_SMPR_7_5_CYCLES
                  && adc_config_SAMPLETIME_19_5 == HW_BIT_ADC_SMPR_19_5_CYCLES
                  && adc_config_SAMPLETIME_61_5 == HW_BIT_ADC_SMPR_61_5_CYCLES
                  && adc_config_SAMPLETIME_181_5 == HW_BIT_ADC_SMPR_181_5_CYCLES
                  && adc_config_SAMPLETIME_601_5 == HW_BIT_ADC_SMPR_601_5_CYCLES);
   uint32_t shift = chan;
   ...
   uint32_t val = *smpr;
   shift *= 3;
   val &= ~ (HW_BIT_ADC_SMPR_BITS << shift);
...
static inline adc_config_e getsampletime_adc(volatile struct adc_t *adc, adc_chan_e chan)
{
   ...
   shift *= 3;
   return (*smpr >> shift) & HW_BIT_ADC_SMPR_BITS;
}

static inline int isoverflow_adc(const adc_t *adc)
{
   return (adc->isr & HW_BIT_ADC_ISR_OVR) / HW_BIT_ADC_ISR_OVR;
}

/* Löscht Overflowflag. */
static inline void clear_overflow_adc(adc_t *adc)
{
   adc->isr = HW_BIT_ADC_ISR_OVR;
}
...
static inline void stop_adc(adc_t *adc)
{
   ...
   if ((cr & HW_BIT_ADC_CR_ADSTART) != 0) {
      cr |= HW_BIT_ADC_CR_ADSTP;
      adc->cr = cr;
      while ((adc->cr & HW_BIT_ADC_CR_ADSTP) != 0) ;
   }
}

static inline int isstarted_adc(const adc_t *adc)
{
   return (adc->cr & HW_BIT_ADC_CR_ADSTART) / HW_BIT_ADC_CR_ADSTART;
}
...
static inline int config_seq_adc(adc_t *adc, uint32_t size_part/*1..8 or size_seq*/, uint32_t size_seq/*1..16*/, adc_chan_e chan[size_seq], adc_config_e config)
{
   ...
   static_assert(HW_BIT_ADC_CFGR_RES_12BIT == 0);
   ...
   cfgr &= ~(  HW_BIT_ADC_CFGR_DISCNUM_MASK  // clear Discontinuous mode channel count
               | HW_BIT_ADC_CFGR_DISCEN      // full sequence, no discontinuous mode (sequence-partitions)
               | HW_BIT_ADC_CFGR_AUTDLY      // no auto delay (wait until dr is read)
               | HW_BIT_ADC_CFGR_CONT        // stop after full sequence
               | HW_BIT_ADC_CFGR_EXTEN_MASK  // disable Hardware External trigger (software trigger mode)
               | HW_BIT_ADC_CFGR_EXTSEL_MASK // clear External trigger selection for regular group
               | HW_BIT_ADC_CFGR_ALIGN       // data in dr is right aligned
               | HW_BIT_ADC_CFGR_RES_MASK    // 12-Bit Resolution
               | HW_BIT_ADC_CFGR_DMAEN       // DMA disabled
               // Disable Automatic injected group conversion ?
               | (isSeqPartitioned ? HW_BIT_ADC_CFGR_JAUTO : 0)
            );
   cfgr |=  (  HW_BIT_ADC_CFGR_OVRMOD // DR register contains latest conversion result even in case of an overrun
               // Enable Discontinuous mode and set Discontinuous mode channel count ?
               | (isSeqPartitioned ? HW_BIT_ADC_CFGR_DISCEN + (size_part << HW_BIT_ADC_CFGR_DISCNUM_POS) : 0)
               | ((config & adc_config_MASK_RESOLUTION)
               #if adc_config_POS_RESOLUTION > HW_BIT_ADC_CFGR_RES_POS
                  >> (adc_config_POS_RESOLUTION - HW_BIT_ADC_CFGR_RES_POS))
               #else
                  << (HW_BIT_ADC_CFGR_RES_POS - adc_config_POS_RESOLUTION))
               #endif
            );
   adc->cfgr = cfgr;
   adc->isr  = (  HW_BIT_ADC_ISR_OVR      // clear ADC overrun
                  | HW_BIT_ADC_ISR_EOS    // clear End of regular sequence flag
                  | HW_BIT_ADC_ISR_EOC    // clear End of conversion flag
                  | HW_BIT_ADC_ISR_EOSMP  // clear End of sampling flag
               );
   for (uint32_t i = 0; i <= size_seq; ++i) {
      setsampletime_adc(adc, chan[i], config);
   }
   uint32_t sqr = (size_seq << HW_BIT_ADC_SQR1_L_POS);
   volatile uint32_t *sqr1 = &adc->sqr1;
   for (uint32_t i = 0, pos = HW_BIT_ADC_SQR1_SQ1_POS; i <= size_seq; ++i, pos += HW_BIT_ADC_SQRX_CHANNEL_LENGTH) {
      if (pos > HW_BIT_ADC_SQR1_SQ4_POS) {
         *sqr1++ = sqr;
         sqr = 0;
         pos = 0;
      }
...
static inline int config_contseq_adc(adc_t *adc, uint32_t size_seq/*1..16*/, adc_chan_e chan[size_seq], adc_config_e config)
{
   ...
   if (err) return err;
   adc->cfgr |= HW_BIT_ADC_CFGR_CONT;  // Continuous conversion mode (sequence repeated) after first trigger
   return 0;
}

static inline uint32_t lenseq_adc(const adc_t *adc)
{
   return 1u + ((adc->sqr1 & HW_BIT_ADC_SQR1_L_MASK) >> HW_BIT_ADC_SQR1_L_POS);
}
 ...
 * HW_BIT_ADC_ISR_EOC in das adc->isr Register), dann gibt es einen Overflow. */
static inline int isdata_adc(const adc_t *adc)
{
   return (adc->isr & HW_BIT_ADC_ISR_EOC) / HW_BIT_ADC_ISR_EOC;
}

static inline int iseos_adc(const adc_t *adc)
{
   return (adc->isr & HW_BIT_ADC_ISR_EOS) / HW_BIT_ADC_ISR_EOS;
}
 ...
 *
 * Unchecked Precondition:
 * (adc->cfgr |= HW_BIT_ADC_CFGR_JAUTO) == 0 // d.h nicht mit config_autojseq_adc konfiguriert
 */
static inline void startj_adc(adc_t *adc)
{
   adc->cr |= HW_BIT_ADC_CR_JADSTART;
}
...
static inline int config_autojseq_adc(adc_t *adc, uint32_t size_jseq/*1..4*/, adc_chan_e chan[size_jseq], adc_config_e config)
{
   ...
   adc->cfgr |= HW_BIT_ADC_CFGR_JAUTO; // Enable Automatic injected group conversion
   return 0;
}
...
static inline int config_jseq_adc(adc_t *adc, uint32_t size_part/*1 or size_jseq*/, uint32_t size_jseq/*1..4*/, adc_chan_e chan[size_jseq], adc_config_e config)
{
   -- size_jseq;
   -- size_part;
   static_assert(HW_BIT_ADC_JSQR_JL_MASK == 3u);
   ...
   uint32_t cfgr = adc->cfgr;
   static_assert(HW_BIT_ADC_CFGR_RES_12BIT == 0);
   cfgr &= ~(  HW_BIT_ADC_CFGR_JAUTO      // Disable Automatic injected group conversion
               | HW_BIT_ADC_CFGR_JQM      // Mode 0: The Queue is never empty and maintains the last written configuration into JSQR.
               | HW_BIT_ADC_CFGR_JDISCEN  // Disable Discontinuous mode on injected channels
               | HW_BIT_ADC_CFGR_AUTDLY   // no auto delay (wait until dr is read)
               | HW_BIT_ADC_CFGR_ALIGN    // data in dr is right aligned
               | HW_BIT_ADC_CFGR_RES_MASK // 12-Bit Resolution
               | HW_BIT_ADC_CFGR_DMAEN    // DMA disabled
   ...
   cfgr |= (   HW_BIT_ADC_CFGR_OVRMOD     // DR register contains latest conversion result even in case of an overrun
               | (size_part != size_jseq ? HW_BIT_ADC_CFGR_JDISCEN : 0)  // Enable Discontinuous mode ?
               | ((config & adc_config_MASK_RESOLUTION)
               #if adc_config_POS_RESOLUTION > HW_BIT_ADC_CFGR_RES_POS
                  >> (adc_config_POS_RESOLUTION - HW_BIT_ADC_CFGR_RES_POS))
               #else
   ...
   adc->cfgr = cfgr;
   adc->isr  = (  HW_BIT_ADC_ISR_JQOVF    // clear Injected context queue overflow
                  | HW_BIT_ADC_ISR_JEOS   // clear Injected channel end of sequence flag
                  | HW_BIT_ADC_ISR_JEOC   // clear Injected channel end of conversion flag
               );
   for (uint32_t i = 0; i <= size_jseq; ++i) {
      setsampletime_adc(adc, chan[i], config);
   }
   uint32_t jsqr = (size_jseq); // J-Seq-Length + Software-Trigger
   static_assert( HW_BIT_ADC_JSQR_JSQ2_POS - HW_BIT_ADC_JSQR_JSQ1_POS == 6
                  && HW_BIT_ADC_JSQR_JSQ3_POS - HW_BIT_ADC_JSQR_JSQ2_POS == 6
                  && HW_BIT_ADC_JSQR_JSQ4_POS - HW_BIT_ADC_JSQR_JSQ3_POS == 6);
   for (uint32_t i = 0, pos = HW_BIT_ADC_JSQR_JSQ1_POS; i <= size_jseq; ++i, pos += 6) {
...
{
   return 1u + ((adc->jsqr & HW_BIT_ADC_JSQR_JL_MASK) >> HW_BIT_ADC_JSQR_JL_POS);
}

static inline int isjdata_adc(const adc_t *adc)
{
   return (adc->isr & HW_BIT_ADC_ISR_JEOC) / HW_BIT_ADC_ISR_JEOC;
}

static inline int isjeos_adc(const adc_t *adc)
{
   return (adc->isr & HW_BIT_ADC_ISR_JEOS) / HW_BIT_ADC_ISR_JEOS;
}

static inline void clear_jdata_adc(adc_t *adc)
{
   adc->isr = HW_BIT_ADC_ISR_JEOC;
}

static inline void clear_jeos_adc(adc_t *adc)
{
   adc->isr = HW_BIT_ADC_ISR_JEOS;
}
