...
#define TIMER6    ((basictimer_t*) HW_REGISTER_BASEADDR_TIM6)  // TIM6
#define TIMER7    ((basictimer_t*) HW_REGISTER_BASEADDR_TIM7)  // TIM7
...
#define HW_REGISTER_BIT_TIM6_CR1_UIFREMAP    (1u << 11)
// Auto-reload preload enable. 1: ARR register is buffered.
#define HW_REGISTER_BIT_TIM6_CR1_ARPE        (1u << 7)   // 0: ARR register is not buffered.
// One-pulse mode. 1: Counter stops counting at the next update event (clearing the CEN bit)
#define HW_REGISTER_BIT_TIM6_CR1_OPM         (1u << 3)   // 0: Counter is not stopped at update event
// Update request source. 1: Only counter overflow generates an update interrupt or DMA request if enabled.
#define HW_REGISTER_BIT_TIM6_CR1_URS         (1u << 2)   // 0: Any of the following events generates interrupt and/or DMA
...
#define HW_REGISTER_BIT_TIM6_CR1_UDIS        (1u << 1)   // 0: The Update (UEV) event is generated by one of the following events:
                                                         // 1. Counter overflow 2. Setting the UG bit 3. Update generation through the slave mode controller
                                                         // Buffered registers are then loaded with their preload values.
// Counter enable. 1: Counter enabled
#define HW_REGISTER_BIT_TIM6_CR1_CEN         (1u << 0)   // 0: Counter disabled
...
#define HW_REGISTER_BIT_TIM6_CR2_MMS_POS     4u
#define HW_REGISTER_BIT_TIM6_CR2_MMS_BITS    0x7
#define HW_REGISTER_BIT_TIM6_CR2_MMS_MASK    HW_REGISTER_MASK(TIM6, CR2, MMS)

// DMA/Interrupt enable register
// Update DMA request enable. 1: Update DMA request enabled.
#define HW_REGISTER_BIT_TIM6_DIER_UDE        (1u << 8)   // 0: Update DMA request disabled.
// Update interrupt enable. 1: Update interrupt enabled.
#define HW_REGISTER_BIT_TIM6_DIER_UIE        (1u << 0)   // 0: Update interrupt disabled.
...
#define HW_REGISTER_BIT_TIM6_SR_UIF          (1u << 0)   // 0: No update occurred.
...
#define HW_REGISTER_BIT_TIM6_EGR_UG          (1u << 0)
...
{
   static_assert(TIMER6_BIT == HW_REGISTER_BIT_RCC_APB1ENR_TIM6EN);
   static_assert(TIMER7_BIT == HW_REGISTER_BIT_RCC_APB1ENR_TIM7EN);
}
...
{
   return (timer->cr1 & HW_REGISTER_BIT_TIM6_CR1_CEN) / HW_REGISTER_BIT_TIM6_CR1_CEN;
}
...
{
   timer->cr1 &= ~HW_REGISTER_BIT_TIM6_CR1_CEN; // disable
...
{
   timer->cr1 |= HW_REGISTER_BIT_TIM6_CR1_CEN;  // enable
}
...
{
   timer->egr = HW_REGISTER_BIT_TIM6_EGR_UG; // generate update event without interrupt/dma
                                             // but reset internal counter (set to 0)
                                             // and reload shadow register (HW_REGISTER_BIT_TIM6_CR1_UDIS not set)
}
...
{
   timer->cr1 &= ~HW_REGISTER_BIT_TIM6_CR1_URS; // setting UG Bit updates shadow register AND generates interrupt/dma
   timer->egr = HW_REGISTER_BIT_TIM6_EGR_UG;    // generate update event
   timer->cr1 |= HW_REGISTER_BIT_TIM6_CR1_URS;  // setting UG Bit updates shadow register but generates no interrupt/dma
}
...
static inline int config_basictimer(basictimer_t *timer, uint32_t period/*2..65536*/, uint32_t prescale/*1..65536*/, basictimercfg_e config)
{
   ...
   timer->cr1 = (0 & ~(
                  HW_REGISTER_BIT_TIM6_CR1_UDIS    // Update Event is generated (not disabled)
                | HW_REGISTER_BIT_TIM6_CR1_OPM     // timer reset at update event and not stopped
                | HW_REGISTER_BIT_TIM6_CR1_CEN))   // stop timer
              | ((config&basictimercfg_ONCE) * HW_REGISTER_BIT_TIM6_CR1_OPM) // counter stops at the next update event
              | HW_REGISTER_BIT_TIM6_CR1_ARPE      // timer->arr is buffered (new values from updateperiod are used only after update event)
              | HW_REGISTER_BIT_TIM6_CR1_URS       // setting UG Bit updates shadow register but generates no interrupt/dma
              | HW_REGISTER_BIT_TIM6_CR1_UIFREMAP  // Interrupt Flag mapped into cnt register (bit 31)
              ;
   static_assert((basictimercfg_MASK_TRIGOUT & 0x03) == 0);
   static_assert((basictimercfg_MASK_TRIGOUT >> 2 & 0x3) == 3);
   timer->cr2 = (config&basictimercfg_MASK_TRIGOUT) << (HW_REGISTER_BIT_TIM6_CR2_MMS_POS - 2);
   timer->sr = 0; // clear any pending interrupt
   timer->dier = ((config&basictimercfg_INTERRUPT) ? HW_REGISTER_BIT_TIM6_DIER_UIE : 0)
               | ((config&basictimercfg_DMA) ? HW_REGISTER_BIT_TIM6_DIER_UDE : 0) ;
   timer->psc = prescale;
   timer->arr = period;
   return 0;
}
...
static inline basictimercfg_e getconfig_basictimer(const volatile struct basictimer_t *timer)
{
   ...
   config  = (timer->cr1 & HW_REGISTER_BIT_TIM6_CR1_OPM) / HW_REGISTER_BIT_TIM6_CR1_OPM;
   config |= (timer->cr2 & HW_REGISTER_BIT_TIM6_CR2_MMS_MASK) >> (HW_REGISTER_BIT_TIM6_CR2_MMS_POS - 2);
   r = timer->dier;
   config |=  (r&HW_REGISTER_BIT_TIM6_DIER_UIE ? basictimercfg_INTERRUPT : 0);
   config |=  (r&HW_REGISTER_BIT_TIM6_DIER_UDE ? basictimercfg_DMA : 0);
...
{
   return (timer->dier & HW_REGISTER_BIT_TIM6_DIER_UIE) != 0;
}
...
{
   timer->dier |= HW_REGISTER_BIT_TIM6_DIER_UIE;
}
...
{
   timer->dier |= HW_REGISTER_BIT_TIM6_DIER_UDE;
}
