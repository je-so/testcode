...
/* == Memory Map == */
#define HW_BASEADDR_SYSTEM    0xE0000000  /* Cortex-M4 System address range 0xE0000000 to 0xFFFFFFFF:
...
#define HW_BASEADDR_DWT       0xE0001000  /* Cortex-M4 Data Watchpoint and Trace (0xE0001000 .. 0xE0001FFF) */
#define HW_BASEADDR_SCS       0xE000E000  /* Cortex-M4 System Control Space contains core peripherals (0xE000E000 .. 0xE000EFFF) */
#define HW_BASEADDR_SYSTICK   0xE000E010  /* Cortex-M4 Systick timer base address */
#define HW_BASEADDR_NVIC      0xE000E100  /* Cortex-M4 Nested Vectored Interrupt Controller */
#define HW_BASEADDR_SCB       0xE000ED00  /* Cortex-M4 System Bontrol Block */
#define HW_BASEADDR_MPU       0xE000ED90  /* Cortex-M4 Memory Protection Unit */
#define HW_BASEADDR_COREDEBUG 0xE000EDF0  /* Cortex-M4 Core Debug Base Address */
#define HW_BASEADDR_FPU       0xE000EF30  /* Cortex-M4 Floating Point Unit */

/* == HW-Units == */
#define hCORE     ((core_sys_t*) HW_BASEADDR_SYSTEM)
#define hSCS      ((core_scs_t*) HW_BASEADDR_SCS)
#define hSCB      ((core_scb_t*) HW_BASEADDR_SCB)

/* == Typen == */

typedef volatile struct core_scs_t     core_scs_t;
typedef volatile struct core_systick_t core_systick_t;
typedef volatile struct core_nvic_t    core_nvic_t;
typedef volatile struct core_scb_t     core_scb_t;
typedef volatile struct core_mpu_t     core_mpu_t;
typedef volatile struct core_debug_t   core_debug_t;
typedef volatile struct core_fpu_t     core_fpu_t;
typedef volatile struct core_sys_t     core_sys_t;

typedef enum core_priority_e {
   core_priority_NONE = 0,          // same as interrupt_priority_MAX
   core_priority_HIGH = 1,          // same as interrupt_priority_HIGH
   core_priority_MID  = ((1<<HW_KONFIG_NVIC_INTERRUPT_PRIORITY_NROFBITS)-1) / 2, // same as interrupt_priority_MID
   core_priority_LOW  = ((1<<HW_KONFIG_NVIC_INTERRUPT_PRIORITY_NROFBITS)-1) - 1, // same as interrupt_priority_LOW
   core_priority_MIN  = ((1<<HW_KONFIG_NVIC_INTERRUPT_PRIORITY_NROFBITS)-1),     // same as interrupt_priority_MIN
} core_priority_e;

#define core_priority_INCR                (-1)
#define core_priority_DECR                (1)
#define ishigher_corepriority(P1, P2)     ((P1) < (P2))
#define ishighequal_corepriority(P1, P2)  ((P1) <= (P2))

/* == Funktion == */

// core: Sleep mode / Power save mode
// TODO: add support for deep sleep states !
static inline void waitinterrupt_core(void); // Enter sleep mode (see WFI) and wakes up with next interrupt (effects of PRIMASK (setprio0mask_interrupt) are ignored. A pending interrupt which would execute if PRIMASK would be cleared will wake-up CPU).
...
static inline void clearevent_core(void);    // Reset cpu-internal event flag to 0.

// core: Priority management (uses same mask as setprioritymask_interrupt)
static inline core_priority_e getpriority_core(void);
            // Gets priority of running thread from register BASEPRI. Only interrupts with higher priority can interrupt the running thread.
static inline void setpriority_core(core_priority_e priority);
            // Sets priority of running thread (uses BASEPRI). Only interrupts with higher priority can interrupt the running thread.
static inline void setprioritymax_core(core_priority_e priority);
            // Sets priority of running thread (uses BASEPRI_MAX) to: setpriority_core( getpriority_core() == core_priority_NONE ? priority : priority == core_priority_NONE ? getpriority_core() : min(priority, getpriority_core())).
            // So that only interrupts with higher priority than parameter priority and old value of getpriority_core()
            // can interrupt the running thread.

// core: Reset
static inline void reset_core(void);

// core: FPU   ; TODO: move fpu into own module
...
#define HW_BIT(BASE, REG, NAME) \
         HW_BIT_ ## BASE ##_## REG ##_## NAME
            // HW_BIT is used to generate the name for value of bitfield NAME of register REG relative to base memory address BASE.

#define HW_OFF(BASE, REG) \
         HW_OFF_ ## BASE ##_## REG
            // HW_OFF is used to generate the name for the offset value of register REG relative to base memory address BASE.

#define HW_BASE(BASE) \
         HW_BASEADDR_ ## BASE
            // HW_BASE is used to generate the name for the base memory address of a certain memory region.

#define HW_SREGISTER(BASE, REG) \
         (((struct { uint8_t _d[HW_OFF_ ## BASE ##_## REG]; volatile uint32_t reg; }*) (HW_BASEADDR_ ## BASE))->reg)
            // HW_SREGISTER is used to access a memory mapped register with offset from base address.

#define HW_REGISTER(BASE, REG) \
         (*((volatile uint32_t*) ((HW_BASEADDR_ ## BASE) + (HW_OFF_ ## BASE ##_## REG))))
...
#define HW_REGISTER_MASK(BASE, REGNAME, FIELD) \
         (HW_BIT_##BASE##_##REGNAME##_##FIELD##_BITS << HW_BIT_##BASE##_##REGNAME##_##FIELD##_POS)
...
#define HW_DEF_OFF(BASE, REG, OFFSET) \
         HW_OFF_ ## BASE ##_## REG = OFFSET
            // HW_DEF_OFF is used to define an offset value for a register relative to some base address.

#define HW_DEF_BIT(BASE, REG, NAME, MSBit, LSBit) \
         HW_BIT_##BASE##_##REG##_##NAME##_POS = LSBit, \
         HW_BIT_##BASE##_##REG##_##NAME##_MAX = ((((1u << ((MSBit) - HW_BIT_##BASE##_##REG##_##NAME##_POS))-1u)<<1u)+1u), \
         HW_BIT_##BASE##_##REG##_##NAME       = HW_BIT_##BASE##_##REG##_##NAME##_MAX << HW_BIT_##BASE##_##REG##_##NAME##_POS
            // HW_DEF_BIT is used to define the value for bitfield NAME of register REG relative to some base address BASE.
            // Parameter 0 <= MSBit <= 31 is the number of the most significant bit of the bit field.
            // Parameter 0 <= LSBit <= MSBIT is the number of the least significant bit of the bit field.
            // The value HW_BIT(BASE, REG, NAME) is defined as bit-mask where every bit between MSBit and LSBit is 1 all other 0.
            // The value HW_BIT(BASE, REG, NAME_POS) equals LSBit which is the position of the first set bit in HW_BIT(BASE, REG, NAME).
            // The value HW_BIT(BASE, REG, NAME_MAX) equals value HW_BIT(BASE, REG, NAME) shifted HW_BIT(BASE, REG, NAME_POS) bit positions to the right.
...
struct core_debug_t {        /* These registers are not reset by a system reset.
                              * They are only reset by a power-on reset.
                              */
...
struct core_sys_t {
   ...
   uint32_t       _reserved0[(HW_BASEADDR_SCS - HW_BASEADDR_SYSTEM) / sizeof(uint32_t)];

   // startaddr: HW_BASEADDR_SCS (Privileged)
   core_scs_t     scs;

   uint32_t       _reserved1[(HW_BASEADDR_SYSTICK - HW_BASEADDR_SCS - 0x0C) / sizeof(uint32_t)];

   // startaddr: HW_BASEADDR_SYSTICK (Privileged)
   core_systick_t systick;

   uint32_t       _reserved2[(HW_BASEADDR_NVIC - HW_BASEADDR_SYSTICK - 0x10) / sizeof(uint32_t)];

   // startaddr: HW_BASEADDR_NVIC (Privileged)
   core_nvic_t    nvic;

   uint32_t       _reserved3[(HW_BASEADDR_SCB - HW_BASEADDR_NVIC - 0x3F0) / sizeof(uint32_t)];

   // startaddr: HW_BASEADDR_SCB (Privileged)
   core_scb_t     scb;

   uint32_t       _reserved4[(HW_BASEADDR_MPU - HW_BASEADDR_SCB - 0x08C) / sizeof(uint32_t)];

   // startaddr: HW_BASEADDR_MPU (Privileged)
   core_mpu_t     mpu;

   uint32_t       _reserved5[(HW_BASEADDR_COREDEBUG - HW_BASEADDR_MPU - 0x2C) / sizeof(uint32_t)];

   // startaddr: HW_BASEADDR_COREDEBUG (Privileged)
   core_debug_t   debug;

   uint32_t       _reserved6[(0xE000EF00 - HW_BASEADDR_COREDEBUG - 0x10) / sizeof(uint32_t)];
   ...
   uint32_t       _reserved7[(HW_BASEADDR_FPU - 0xE000EF00 - 0x04) / sizeof(uint32_t)];

   // startaddr: HW_BASEADDR_FPU (Privileged)
   core_fpu_t     fpu;
...
{
   static_assert( offsetof(core_sys_t, scs)        == HW_BASEADDR_SCS - HW_BASEADDR_SYSTEM);
   ...
   static_assert( HW_BIT_SCS_ICTR_INTLINESNUM     == 15);
   static_assert( HW_BIT_SCS_ICTR_INTLINESNUM_POS == 0);
   static_assert( HW_BIT_SCS_ICTR_INTLINESNUM_MAX == 15);
   static_assert( offsetof(core_sys_t, systick)       == HW_BASEADDR_SYSTICK - HW_BASEADDR_SYSTEM);
   ...
   static_assert( offsetof(core_sys_t, systick.calib) == HW_BASEADDR_SYSTICK - HW_BASEADDR_SYSTEM + HW_OFF(SYSTICK, CALIB));
   ...
   static_assert( offsetof(core_sys_t, nvic)       == HW_BASEADDR_NVIC - HW_BASEADDR_SYSTEM);
   ...
   static_assert( offsetof(core_sys_t, scb)        == HW_BASEADDR_SCB - HW_BASEADDR_SYSTEM);
   ...
   static_assert( offsetof(core_sys_t, mpu)        == HW_BASEADDR_MPU - HW_BASEADDR_SYSTEM);
   ...
   static_assert( offsetof(core_sys_t, debug)      == HW_BASEADDR_COREDEBUG - HW_BASEADDR_SYSTEM);
   // TODO: debug
   static_assert( offsetof(core_sys_t, stir)       == offsetof(core_sys_t, scs) + HW_OFF(SCS, STIR));
   static_assert( HW_BIT(SCS, STIR, INTID)         == 0x1ff);
   static_assert( offsetof(core_sys_t, fpu)        == HW_BASEADDR_FPU - HW_BASEADDR_SYSTEM);
   // TODO: fpu
}
...
/* function: setevent_core
 * Setzt das interne Eventflag auf 0 zurück. */
static inline void clearevent_core(void)
{
   __asm volatile(
         "SEV\n"
         "WFE"
...
{
         uint32_t priority;
         __asm volatile( "mrs %0, BASEPRI" : "=r" (priority) ::);
...
{
         priority <<= (8-HW_KONFIG_NVIC_INTERRUPT_PRIORITY_NROFBITS);
         __asm volatile( "msr BASEPRI_MAX, %0" :: "r" (priority) : );
}
