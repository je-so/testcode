...
int main(void)
{
   ...
   for (int i = 0; i < 10; ++i) {
      setpins_gpio(GPIO_PORTE, mask, GPIO_PINS(15,8)&~mask);
      for (volatile int i = 0; i < 50000; ++i) ;
      mask >>= 1;
      mask = (mask & GPIO_PINS(15,8)) | ((mask & GPIO_PINS(7,0)) << 8);
   }
   setpins_gpio(GPIO_PORTE, 0, GPIO_PINS(15,8));
   ...
   while (1) {
      for (int round = 0; round < 1; ++round) {
         ...
         for (int pin = 8; pin <= 15; ++pin) {
            setpins_gpio(GPIO_PORTE, 0, GPIO_PIN(pin)/*off*/);
            for (volatile int i = 0; i < 50000; ++i) ;
         }
      }
      for (int round = 0; round < 1; ++round) {
         for (int pin = 8; pin <= 15; ++pin) {
            setpins_gpio(GPIO_PORTE, GPIO_PIN(pin)/*on*/, 0);
            for (volatile int i = 0; i < 50000; ++i) ;
         }
         for (int pin = 15; pin >= 8; --pin) {
