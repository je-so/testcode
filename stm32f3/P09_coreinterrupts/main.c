      ...
      case coreinterrupt_SYSTICK:
         ...
         if ( i == coreinterrupt_MPUFAULT
            ...
            if (i == coreinterrupt_DEBUGMONITOR) {
               // Interrupt wird trotz an- oder ausgeschaltetem Zustand ausgeführt
               // jedoch wird aktiviert Debugevent einen debugmonitor_interrupt,
               // zur Implementierung eines SW-Debuggers, falls der HW-Debugger nicht angeschlossen
               // (isdebugger_dbg() == 0)
               if (enable_coreinterrupt(i)) goto ONERR;
               if (disable_coreinterrupt(i)) goto ONERR;
            } else {
